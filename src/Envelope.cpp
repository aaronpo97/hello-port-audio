...
float Envelope::processEnvelope()
{
  ...
  {
    float const attackTimeSec =
        static_cast<float>(m_attackTimeMs.load(std::memory_order_relaxed)) /
        1000.0f;
    ...
    else
    {
      ...
      m_stageTime.store(stageTime + sampleTime, std::memory_order_relaxed);
    }
    break;
  }
  ...
  {
    float const decayTimeSec =
        static_cast<float>(m_decayTimeMs.load(std::memory_order_relaxed)) /
        1000.0f;
    float const sustainLevel = m_sustainLevel.load(std::memory_order_relaxed);

    // Calculate how much to decay from peak to get to sustain level
    float const decayAmount =
        (1.0f - sustainLevel) * (stageTime / std::max(decayTimeSec, 0.001f));
    ...
    if (currentAmplitude <= sustainLevel || stageTime >= decayTimeSec)
    {
      ...
      m_stage.store(EnvelopeStage::Sustain, std::memory_order_relaxed);
      m_stageTime.store(0.0f, std::memory_order_relaxed);
    }
    else
    {
  ...
  case EnvelopeStage::Release:
  {
    float const releaseTimeSec =
        static_cast<float>(m_releaseTimeMs.load(std::memory_order_relaxed)) /
        1000.0f;
